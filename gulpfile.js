// プロジェクト名
const projectName = 'fight-on-height';
...
const mcdir =
  os.homedir() + '/AppData/Local/Packages/Microsoft.MinecraftUWP_8wekyb3d8bbwe/LocalState/games/com.mojang/';
...
function deploy_localmc_behavior_pack() {
  return gulp.src(['build/behavior_pack/**/*']).pipe(gulp.dest(mcdir + 'development_behavior_packs/' + projectName));
}
function deploy_localmc_resource_pack() {
  return gulp.src(['build/resource_pack/**/*']).pipe(gulp.dest(mcdir + 'development_resource_packs/' + projectName));
}
